...
class InputParameters:
	...
	Attributes:
		...
		Delta (float): Plasma triangularity. Positive is conventional D-shape, negative is inverse D-shape. Default: 0.0
		PlasmaT (float): Plasma temperature in keV. Default: 15.0
		SafetyFac (float): Plasma safety factor (q_edge / q95-style engineering input). Default: 3.5
...
def _miller_boundary(r_major: float, r_minor: float, kappa: float, delta: float, npts: int = 2048):
	"""Return a Miller-like LCFS boundary parameterisation."""
	theta = np.linspace(0.0, 2.0*np.pi, npts, endpoint=False)
	asind = np.arcsin(np.clip(delta, -0.999999, 0.999999))
	angle = theta + asind * np.sin(theta)
	r = r_major + r_minor * np.cos(angle)
	z = kappa * r_minor * np.sin(theta)
	return theta, r, z


def _closed_curve_area(r: np.ndarray, z: np.ndarray) -> float:
	r_next = np.roll(r, -1)
	z_next = np.roll(z, -1)
	return 0.5 * np.abs(np.sum(r * z_next - r_next * z))


def _boundary_perimeter(r: np.ndarray, z: np.ndarray) -> float:
	dr = np.roll(r, -1) - r
	dz = np.roll(z, -1) - z
	return np.sum(np.sqrt(dr**2 + dz**2))


def _surface_of_revolution(r: np.ndarray, z: np.ndarray) -> float:
	r_next = np.roll(r, -1)
	z_next = np.roll(z, -1)
...
def _shape_metrics(r_major: float, r_minor: float, kappa: float, delta: float):
	_, r, z = _miller_boundary(r_major, r_minor, kappa, delta)
...
def _sauter_ip_shape_factor(kappa: float, delta: float, squareness: float = 1.0) -> float:
	"""
...
def _bootstrap_enhancement(inv_aspect: float, delta: float) -> float:
	"""
	Approximate bootstrap enhancement from triangularity using a trapped-particle based effective inverse aspect ratio.
	The shaping effect is evaluated at a representative inner flux surface (delta75 ~ delta/2).
	"""
	epsilon = np.clip(inv_aspect, 1.0e-6, 0.95)
	delta75 = 0.5 * delta
	eps_eff = np.clip(0.67 * (1.0 - 1.4 * delta75 * abs(delta75)) * epsilon, 1.0e-6, 0.95)

	def trapped_fraction(eps: float) -> float:
		return 1.0 - ((1.0 - eps) / (1.0 + 2.0 * np.sqrt(eps))) * np.sqrt((1.0 - epsilon) / (1.0 + epsilon))

	f0 = trapped_fraction(0.67 * epsilon)
	fd = trapped_fraction(eps_eff)
	return max((fd / (1.0 - fd)) / (f0 / (1.0 - f0)), 0.0)


def _stable_kappa_estimate(aspect: float, delta: float) -> float:
	"""A mild shaping-aware vertical controllability proxy."""
	base = 1.46 + 0.5 / max(aspect - 1.0, 0.2)
	# Positive triangularity modestly helps controllability, negative triangularity modestly hurts it.
	return base * (1.0 + 0.12 * delta)
...
def simplesystemcode(inputs: InputParameters, print_out=True):
	...
	# Bound triangularity to a physically sensible range for Miller geometry.
	...
	# Shaped geometry from a Miller-like LCFS.
	...
	MagField = inputs.BMax * (RMajor - RMinor - BlanketThickness) / RMajor   # Magnetic field on the magnetic axis, T
	...
	BootFrac = BootDrive / (1.0 + BootDrive) # Bootstrap fraction, simple shaped correction with saturation
	...
	StableKappa = _stable_kappa_estimate(Aspect, delta)  # Estimated maximum vertically-controllable kappa, shaped proxy
	nG = PlasCur / PlasCrossSection # Greenwald density limit, 10^20 m-3
	BetaNLimit = 3.9 + 0.8 * delta  # Simple shaped no-wall proxy matching ~4.3 at delta=+0.5 and ~3.5 at delta=-0.5
	...
	out_dict["ShapeFac"] = ShapeFac
	out_dict["BootEnhancement"] = BootEnhancement
	out_dict["BootFrac"] = BootFrac # The higher, the better
	out_dict["PlasCur"] = PlasCur
	out_dict["PlasVol"] = PlasVol
	out_dict["PlasSurf"] = PlasSurf
	out_dict["PlasCrossSection"] = PlasCrossSection
	out_dict["PlasPerimeter"] = PlasPerimeter
	...
	out_dict["BetaPol"] = BetaPol
	out_dict["betaN"] = 100.0 * Beta * RMinor * MagField / PlasCur # Below ~4.5 or so
	out_dict["betaNLimit"] = BetaNLimit
	out_dict["betaNMargin"] = BetaNLimit - out_dict["betaN"]
	...
	out_dict["DeltaClipped"] = float(delta != inputs.Delta)
	...
	if print_out:
		...
		print('Plasma triangularity: {:2.2f}'.format(delta))
		...
		print('Bootstrap enhancement: {:2.2f}'.format(BootEnhancement))
		print('Plasma current: {:2.2f} MA'.format(PlasCur))
		print('Plasma volume: {:2.2f} m3'.format(PlasVol))
		print('Plasma surface area: {:2.2f} m2'.format(PlasSurf))
		print('Plasma perimeter: {:2.2f} m'.format(PlasPerimeter))
		print("")
		...
		print('Normalised beta: {:2.2f} % m T MA-1'.format(100.0 * Beta * RMinor * MagField / PlasCur))
		print('Triangularity-shaped beta limit proxy: {:2.2f}'.format(BetaNLimit))
